...
class PostFactory {
    ...
    {
        PostFactory::validate($p);
        $post_type = gettype($p['post_type']) == 'string' ? $p['post_type'] : 'post';
        return new Post(
            id: $p['id'] ?? 0,
            post_author: $p['post_author'] ?? get_current_user_id(),  // The ID of the user who added the post. Default is the current user ID.
            post_date: $p['post_date'] ?? new DateTimeImmutable('Y-m-d H:i:s', DateTimeZone::UTC),  // The date of the post. Default is the current time.
            post_date_gmt: $p['post_date_gmt'] ?? new DateTimeImmutable('Y-m-d H:i:s', DateTimeZone::UTC),  // The date of the post in the GMT timezone. Default is the value of $post_date.
            ...
            post_modified: $p['post_modified'] ?? new DateTimeImmutable('Y-m-d H:i:s', DateTimeZone::UTC),  // The date when the post was last modified. Default is the current time.
            post_modified_gmt: $p['post_modified_gmt'] ?? new DateTimeImmutable('Y-m-d H:i:s', DateTimeZone::UTC),  // The date when the post was last modified in the GMT timezone. Default is the current time.
    ...
    private static function validate(array $p) {
        PostFactory::validate_key('id', 'int', $p);
        PostFactory::validate_key('post_author', 'int', $p);
        PostFactory::validate_key('post_date', 'string', $p);
        PostFactory::validate_key('post_date_gmt', 'string', $p);
        PostFactory::validate_key('post_content', 'string', $p);
        PostFactory::validate_key('post_content_filtered', 'string', $p);
        PostFactory::validate_key('post_title', 'string', $p);
        PostFactory::validate_key('post_excerpt', 'string', $p);
        PostFactory::validate_key('post_status', 'string', $p);
        PostFactory::validate_key('post_type', 'string', $p);
        PostFactory::validate_key('comment_status', 'string', $p);
        PostFactory::validate_key('ping_status', 'string', $p);
        PostFactory::validate_key('post_password', 'string', $p);
        PostFactory::validate_key('post_name', 'string', $p);
        PostFactory::validate_key('to_ping', 'string', $p);
        PostFactory::validate_key('pinged', 'string', $p);
        PostFactory::validate_key('post_modified', 'string', $p);
        PostFactory::validate_key('post_modified_gmt', 'string', $p);
        PostFactory::validate_key('post_parent', 'int', $p);
        PostFactory::validate_key('menu_order', 'int', $p);
        PostFactory::validate_key('post_mime_type', 'string', $p);
        PostFactory::validate_key('guid', 'string', $p);
        PostFactory::validate_key('import_id', 'int', $p);
        PostFactory::validate_key('post_category', 'int', $p);
        PostFactory::validate_key('tags_input', 'array', $p);
        PostFactory::validate_key('tax_input', 'array', $p);
        PostFactory::validate_key('meta_input', 'array', $p);
        PostFactory::validate_key('page_template', 'string', $p);
    }

    private static function validate_key(string $key, string $expected_type, array $array): void
    {
        if ( array_key_exists($key, $array) ) {
            $type = gettype($array[$key]);if ($type != $expected_type) { throw new ValueError("Property $key is of the wrong type.  Expected $expected_type got $type.");}
        }
    }
}